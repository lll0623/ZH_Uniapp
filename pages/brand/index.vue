    ...
    <div class="brand-wrapper">
        ...
        <div class="content">
            <ul class="clearfix bgCW top statistics">
                <li class="tc fl rel">
                    <p class="fs16 c3">{{ MyBrand | formatMoney(0) }}</p>
                    <p class="fs10 c9 mt4">我的品牌</p>
                </li>
                <li class="tc fl rel">
                ...
                <li class="tc fl rel">
                    <p class="fs16 c3">{{ MyFavoriteBrand | formatMoney(0) }}</p>
                    <p class="fs10 c9 mt4">我的收藏</p>
                </li>
            </ul>
            <!-- 列表 -->
            <ul class="nav clearfix">
                <li :class="navActive === 0 ? 'active' : ''" @click="changeNavFn(0)">全部品牌</li>
            ...
            <ul class="lists" :class="navActive === 0 ? 'block' : 'none'">
                ...
                <li @click="goBrandDetailFn(item.Id)" v-for="(item, idx) in BrandList" :key="idx">
            ...
            <ul class="lists" :class="navActive === 1 ? 'block' : 'none'">
                <li class="tc cRed" v-if="MyPrivateBrandList.length == 0">暂无数据</li>
                <li @click="goBrandDetailFn(item.Id)" v-for="(item, idx) in MyPrivateBrandList" :key="idx">
                    <div class="clearfix top">
                        <h4 class="fl fs14 c0">{{ item.BrandName }}</h4>
                        <div class="fl tag-wrap">
                            <van-tag class="tag" v-if="item.BrandStatus != ''" size="medium" type="primary">{{ item.BrandStatus }}</van-tag>
                    ...
                    <div class="clearfix mt8 fs14 c9">
                        <p class="fl">{{ item.Format }}</p>
                        <p class="fl" v-if="item.Area !=''">/{{ item.Area | GetItemsFilter('Ls.Ag.ConstructionArea', globalData.windowData.GetItemsData) }}</p>
                    </div>
                    <div class="clearfix mt8 fs14 c9" v-if="item.Company !=''">{{ item.Company }}</div>
                </li>
            </ul>
            <ul class="lists" :class="navActive === 2 ? 'block' : 'none'">
                ...
                <li @click="goBrandDetailFn(item.Id)" v-for="(item, idx) in MyFavoriteList" :key="idx">
                    <div class="clearfix top">
                        <h4 class="fl fs14 c0">{{ item.BrandName }}</h4>
                        <div class="fl tag-wrap">
                            <van-tag class="tag" v-if="item.BrandStatus != ''" size="medium" type="primary">{{ item.BrandStatus }}</van-tag>
        ...
        <van-popup :show="isShow" closeable position="right" custom-style="height: 100%;width:280px;padding-bottom: 60px;" @close="onCloseFilterFn">
            <div class="filter-wrapper">
                ...
                <div class="content">
                    <div class="item"><block-sec v-if="hackReset" @listenTochildEvent="listenTochildFun" label="品牌状态" dataList="true" lsCode="CustStatus" /></div>
                    ...
                    <div class="item"><block-sec v-if="hackReset" @listenTochildEvent="listenTochildFun" label="品牌定位" lsCode="Position" /></div>
                    ...
                    <div class="item"><block-sec v-if="hackReset" @listenTochildEvent="listenTochildFun" label="来源" lsCode="CustSource" /></div>
                </div>
            </div>
        ...
        <div class="common-add-btn fix bgCBlue tc" @click="goAddBrandFn"><van-icon name="plus" class="icon cWhite" /></div>
...
import { BrandLists, FavoriteOrCancel } from '../../utils/http';
...
let paramsJsonChushi = {
    ...
    PageIndex: 1,
    PageSize: 15,
    BrandStatus: '', // 客户状态 筛选用
    IsPrivateCust: '', // 公私客 筛选用
    Urgency: '', // 意向等级 筛选用
    Position: '',
    Area: '', // 面积
    Format: '', // 业态 筛选用
    BrandSource: '', // 客户来源 筛选用
    IsFavorite: '', // 是否收藏 筛选用
...
export default {
    ...
    onShow() {
        ...
        if (this.globalData.windowData.Brand) {
            this.Refresh();
            this.globalData.windowData.Brand = false;
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        listenTochildFun(val, name) {
            ...
            if (val[0].FormatCode == -1) {
                if (name == 'CustStatus') {
                    this.paramsJson['BrandStatus'] = '';
                } else if (name == 'ConstructionArea') {
                    this.paramsJson['Area'] = '';
                } else if (name == 'IsImPrivateCust') {
                    this.paramsJson['IsPrivateCust'] = '';
                } else if (name == 'CustSource') {
                    this.paramsJson['BrandSource'] = '';
                } else {
                    this.paramsJson[name] = '';
                }
            } else {
                if (name == 'CustStatus') {
                    this.paramsJson['BrandStatus'] = val[0].FormatCode;
                } else if (name == 'ConstructionArea') {
                    this.paramsJson['Area'] = val[0].FormatCode;
                } else if (name == 'IsImPrivateCust') {
                    this.paramsJson['IsPrivateCust'] = val[0].FormatCode;
                } else if (name == 'CustSource') {
                    this.paramsJson['BrandSource'] = val[0].FormatCode;
                } else {
        ...
        goBrandDetailFn(id) {
            wx.navigateTo({ url: '../brandDetail/index?id=' + id });
        },
        goAddBrandFn() {
            this.globalData.windowData["editBrandData"] = {};
                ...
                BrandLists(params).then(res => {
                    if (res.Status == 200) {
                        if (this.paramsJson.PageIndex == 1) {
                            this.MyBrand = res.Data.MyBrand;
                            this.MyFavoriteBrand = res.Data.MyFavoriteBrand;
                            this.MyPrivateCust = res.Data.MyPrivateCust;
                        }
                        let CusList = (res.Data.MyBrandList == "") ? [] : res.Data.MyBrandList; // 全部品牌列表数据
                        ...
                        this.BrandList = this.BrandList.concat(CusList);
                        this.MyFavoriteList = this.MyFavoriteList.concat(FavoriteList);
                        this.MyPrivateBrandList = this.MyPrivateBrandList.concat(PrivateList);
                    } else {
                        Toast.fail({
                            message: res.Message,
                            position: 'top',
        ...
        isLike (id, isTrue, i) {
            this.BrandList[i].IsFavorite = !isTrue;
            let shoucang = {
                id: id,
                isFavorite: !isTrue,
                refType: 7
            ...
            FavoriteOrCancel(shoucang).then(res => {
                if (res.Status == 200) {
                    ...
                    if (!isTrue) {
                        this.MyFavoriteList.push(this.BrandList[i])
                    } else {
                        this.MyFavoriteList = this.MyFavoriteList.filter(res =>{
                            return res.Id !=id;
                        })
                    }
                    this.MyFavoriteBrand = this.MyFavoriteList.length;
                } else {
